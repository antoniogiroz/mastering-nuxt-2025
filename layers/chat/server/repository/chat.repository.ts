...
export async function createChat(data: { title?: string, projectId?: string }): Promise<ChatWithProject | null> {
  const now = new Date();
  const newChat: Chat = {
    id: uuidv4(),
...
export async function updateChat(id: string, data: { title?: string, projectId?: string }): Promise<ChatWithProject | null> {
...
export async function createMessageForChat(data: { content: string, role: 'user' | 'assistant', chatId: string }): Promise<ChatMessage | null> {
